...
const App: React.FC = () => {
  // appEnv retained for debugging — injected via Vercel env vars
  const _appEnv = (import.meta.env.VITE_APP_ENV || import.meta.env.VITE_ENV || 'DEV').toString().toUpperCase();
  void _appEnv;
  ...
  const submitChat = async (input: string) => {
    ...
    try {
      // Build maximum-richness context from ALL available analysis state
      const repoContext = [
        `=== REPOSITORY ===`,
        `${repo.owner}/${repo.repo} — ${repo.description || 'No description'}`,
        `Stars: ${repo.stars?.toLocaleString() ?? 0} | Forks: ${repo.forks?.toLocaleString() ?? 0} | Language: ${repo.language || 'N/A'} | Topics: ${(repo.topics ?? []).join(', ') || 'none'}`,
        '',
        `=== AI SUMMARY ===`,
        analysis.summary,
        '',
        ...
        '',
        `=== SCORECARD (out of 10) ===`,
        `Maintenance: ${analysis.scorecard.maintenance} | Documentation: ${analysis.scorecard.documentation} | Security: ${analysis.scorecard.security} | Innovation: ${analysis.scorecard.innovation}`,
        '',
        `=== ARCHITECTURE ===`,
        ...
        '',
        `=== ROADMAP ===`,
        ...(analysis.roadmap ?? []).map((r, i) => `${i + 1}. ${r}`),
        '',
        ...(deepAudit ? [
          `=== SECURITY & DEEP AUDIT ===`,
          `Audit Reasoning: ${deepAudit.reasoning}`,
          `Architectural Debt: ${deepAudit.architecturalDebt}`,
          `Vulnerabilities Found (${deepAudit.vulnerabilities.length}):`,
          ...deepAudit.vulnerabilities.map((v, i) => `  ${i + 1}. ${v}`),
        ] : [
          `=== SECURITY ===`,
          `Security Scorecard: ${analysis.scorecard.security}/10 (Deep Audit not yet run — user needs to click "Run Deep Audit" in the Security tab)`,
        ]),
        '',
        ...(insights ? [
          `=== CODE HEALTH ===`,
          `Total Files: ${insights.codeHealth.totalFiles} | Total Lines: ${insights.codeHealth.totalLines?.toLocaleString() ?? 'N/A'}`,
          `Complexity: ${insights.codeHealth.complexity} | Last Commit: ${insights.codeHealth.lastCommit} | Commit Frequency: ${insights.codeHealth.commitFrequency}`,
          `Languages: ${Object.entries(insights.codeHealth.languages).map(([l, n]) => `${l} (${n})`).join(', ')}`,
          '',
          `=== ISSUES SUMMARY ===`,
          insights.issuesSummary.overview,
          ...insights.issuesSummary.sections.map(s => `${s.heading}: ${s.bullets.join('; ')}`),
          '',
          `=== PR SUMMARY ===`,
          insights.prSummary.overview,
          '',
          `=== TEAM DYNAMICS ===`,
          insights.teamDynamics.overview,
          `Top Contributors: ${insights.contributors.slice(0, 5).map(c => `${c.login} (${c.contributions} commits)`).join(', ')}`,
          '',
          `=== DEPENDENCIES ===`,
          `Total: ${insights.dependencies.length} | Outdated: ${insights.dependencies.filter(d => d.isOutdated).length} | Vulnerable: ${insights.dependencies.filter(d => d.vulnerabilities?.length).length}`,
          ...insights.dependencies.filter(d => d.isOutdated || d.vulnerabilities?.length).slice(0, 10).map(d =>
            `  • ${d.name} ${d.currentVersion}${d.isOutdated ? ` (outdated → ${d.latestVersion})` : ''}${d.vulnerabilities?.length ? ` [VULNERABLE: ${d.vulnerabilities.join(', ')}]` : ''}`
          ),
        ] : []),
        '',
        ...(prReviewResult ? [
          `=== LAST PR REVIEW ===`,
          `Risk: ${prReviewResult.riskLevel.toUpperCase()} | Summary: ${prReviewResult.summary}`,
          `Findings: ${prReviewResult.findings.map(f => `${f.severity.toUpperCase()} — ${f.title} (${f.file})`).join('; ')}`,
          `Security Checks: ${prReviewResult.securityChecks.join('; ')}`,
        ] : []),
        '',
        ...(onboardingGuide ? [
          `=== ONBOARDING ===`,
          `Quick Start: ${onboardingGuide.quickStart}`,
          `Critical Files: ${(onboardingGuide.criticalFiles ?? []).slice(0, 10).join(', ')}`,
          ...(onboardingGuide.testingSetup?.hasTests ? [
            `Testing: ${onboardingGuide.testingSetup.testFramework} — run with: ${onboardingGuide.testingSetup.testCommand}`,
            `Test Files: ${(onboardingGuide.testingSetup.testFiles ?? []).slice(0, 5).join(', ')}`,
          ] : ['Testing: No test suite detected']),
          ...(onboardingGuide.recentActivity ? [
            `Recent Activity: ${onboardingGuide.recentActivity.totalCommits} commits, ${onboardingGuide.recentActivity.activeDevs} active devs in last 7 days`,
            `Hot Files: ${(onboardingGuide.recentActivity.hotFiles ?? []).slice(0, 8).map(([f, c]) => `${f} (${c} changes)`).join(', ')}`,
          ] : []),
          ...(onboardingGuide.commonTasks?.length ? [
            `Common Tasks:`,
            ...onboardingGuide.commonTasks.map(t => `  • ${t.task}: ${t.steps.join(' → ')}`),
          ] : []),
          `Setup: ${onboardingGuide.setupInstructions?.slice(0, 600) ?? 'N/A'}`,
        ] : []),
        '',
        ...(analysis.cloudArchitecture?.length ? [
          `=== DEPLOYMENT OPTIONS ===`,
          ...analysis.cloudArchitecture.map(p => `• ${p.serviceName} on ${p.platform} — ${p.reasoning?.slice(0, 200) ?? ''}`),
        ] : []),
        '',
        `=== FILE TREE (first 30 files) ===`,
        ...(structure ?? []).slice(0, 30).map(f => f.path),
      ].join('\n');
    ...
    <div className="min-h-screen bg-[#020617] text-slate-300 font-sans selection:bg-indigo-500/30 overflow-x-hidden">
      ...
      <main className="max-w-[1900px] mx-auto p-4 sm:p-8 xl:p-12 pb-20">
        {loading ? (
          ...
          <div className="grid grid-cols-12 gap-4 sm:gap-6 lg:gap-8">
            ...
            <div className="col-span-12 lg:col-span-8 xl:col-span-9 space-y-6 sm:space-y-10">
      ...
      {analysis && (
        <>
          ...
          {/* Slide-in panel */}
          <div className={`fixed bottom-0 right-6 z-[100] flex flex-col transition-all duration-300 ease-in-out ${chatOpen ? 'h-[680px]' : 'h-0'} w-[400px] max-w-[calc(100vw-3rem)]`}>
            {chatOpen && (
              <div className="flex flex-col h-full bg-slate-900 border border-slate-700 rounded-t-3xl shadow-2xl overflow-hidden">
                {/* Header */}
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-gradient-to-r from-indigo-600/20 to-purple-600/20 shrink-0">
                  <div className="flex items-center gap-3">
                    <BrainCircuit className="w-5 h-5 text-indigo-400" />
                    <span className="text-white font-black text-base tracking-tight">AI Copilot</span>
                    {chatLoading && <span className="text-[10px] text-indigo-400 animate-pulse font-bold uppercase tracking-widest">Thinking...</span>}
                  </div>
                  <div className="flex items-center gap-3">
                    <button onClick={() => setChatHistory([])} className="text-slate-500 hover:text-slate-300 text-xs font-medium transition-colors">Clear</button>
                ...
                <div className="flex-1 flex flex-col min-h-0">
                  ...
                  {chatHistory.length === 0 && (
                    <div className="p-4 space-y-2 shrink-0">
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">💡 Ask anything about this repo</p>
                  ...
                  {/* Messages */}
                  <div className="flex-1 overflow-y-auto p-4 space-y-4 min-h-0" style={{scrollbarWidth:'thin',scrollbarColor:'#334155 transparent'}}>
                        ...
                        <div className={`max-w-[88%] p-3 rounded-2xl text-sm leading-relaxed ${
                          ...
                          {msg.role === 'model' && (
                            <div className="flex items-center gap-2 mb-2 pb-2 border-b border-slate-700/60">
                              ...
                              <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">AI Copilot</span>
                            </div>
                          )}
                          {msg.role === 'model' ? (
                            <div className="text-xs space-y-1.5 leading-relaxed">
                              {msg.text.split('\n').map((line, li) => {
                                ...
                                if (/^[-*•]\s/.test(line)) {
                                  ...
                                  return <div key={li} className="flex gap-1.5 min-w-0"><span className="text-indigo-400 shrink-0">•</span><span className="min-w-0 break-words" dangerouslySetInnerHTML={{ __html: content.replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>').replace(/`([^`]+)`/g, '<code class="bg-black/40 px-1 rounded text-emerald-300 font-mono">$1</code>') }} /></div>;
                                }
                                if (/^\d+\.\s/.test(line)) {
                                  const num = line.match(/^(\d+)\./)?.[1];
                                  const content = line.replace(/^\d+\.\s/, '');
                                  return <div key={li} className="flex gap-1.5 min-w-0"><span className="text-indigo-400 font-bold shrink-0">{num}.</span><span className="min-w-0 break-words" dangerouslySetInnerHTML={{ __html: content.replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>').replace(/`([^`]+)`/g, '<code class="bg-black/40 px-1 rounded text-emerald-300 font-mono">$1</code>') }} /></div>;
                                }
                                if (/^#{1,3}\s/.test(line)) {
                                  const content = line.replace(/^#{1,3}\s/, '');
                                  return <div key={li} className="font-bold text-white mt-2 break-words">{content}</div>;
                                }
                                if (!line.trim()) return <div key={li} className="h-1" />;
                                return <div key={li} className="break-words" dangerouslySetInnerHTML={{ __html: line.replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>').replace(/`([^`]+)`/g, '<code class="bg-black/40 px-1 rounded text-emerald-300 font-mono">$1</code>') }} />;
                              })}
                            </div>
                          ) : (
                          ...
                          {msg.fileLinks && msg.fileLinks.length > 0 && (
                            <div className="mt-3 space-y-1">
                              {msg.fileLinks.map((path) => (
                                <button key={path} onClick={() => openFileByPath(path)} className="w-full text-left px-3 py-2 bg-slate-900/60 border border-slate-700 hover:border-emerald-500/60 rounded-lg text-xs font-mono text-emerald-400 hover:text-emerald-300 transition-all">
                                  {path.split('/').pop()}
                                </button>
                              ))}
                            </div>
                    ...
                    {chatLoading && (
                      <div className="flex justify-start">
                        <div className="bg-slate-800 border border-slate-700/60 rounded-2xl rounded-bl-sm p-3 flex items-center gap-2">
                  ...
                  <form onSubmit={handleChat} className="p-4 border-t border-slate-800 shrink-0">
                    <div className="relative">
                      <input
                        className="w-full bg-slate-800 border border-slate-700 focus:border-indigo-500 rounded-2xl pl-4 pr-12 py-3 text-sm text-white placeholder:text-slate-500 outline-none transition-all"
                      ...
                      <button
                        ...
                        className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 flex items-center justify-center bg-indigo-600 hover:bg-indigo-500 disabled:opacity-30 disabled:cursor-not-allowed rounded-xl transition-all"
                      >
